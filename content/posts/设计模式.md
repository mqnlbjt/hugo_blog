...
当需要创建一个复杂对象的时候会遇到很多问题：

1. 构造函数参数过多
   + 要创建一个 Computer 类，它有很多配置项：CPU、内存、硬盘、显卡、主板、电源...
   + 如果有部分配件是可选项，比如机箱风扇，需要创建多个重载的构造函数。
2. 如果使用无参构造函数Setters方法导致对象状态不稳定
   + 从 new Computer() 到所有 set 方法调用完毕之前，这个 computer 对象处于一个不完整、不一致的状态。如果在中途被其他线程使用，可能会引发不可预料的错误。
   + 无法轻易地将这个对象做成不可变的，因为它的属性需要能被外界修改。
...
1. Product（产品）：
   ...
   + 它通常包含多个组成部分。
2. Builder（抽象建造者）：
   + 一个接口或抽象类，定义了创建 Product 各个部分所需的所有步骤。
   + 它还会提供一个最终获取产品的方法，如 getResult()。
3. ConcreteBuilder（具体建造者）：
   + 实现了抽象 Builder 接口。
   + 它知道如何实现每个构建步骤，并负责组装产品。
4. Director（指挥者，可选）：
   + 这个角色是可选的，但有助于理解分离思想。
   + Director 负责管理正确的构建顺序。它接受一个 Builder 实例，然后按预定顺序（例如，先装主板，再装CPU，最后装内存）调用 Builder 的方法。

### 案例

装修公司都会给出自家的套餐服务，按照不同的套餐的价格选取不同的品牌组合，最终再按照装修面积给出一个整体的报价
...
2. `DecorationPackageMenu`，是`IMenu`接口的实现类，主要是承载建造过程中的填充器。相当于这是一套承载物料和创建者中间衔接的内容。
...
//一行调用
 List detail = new Builder().levelOne(132.52D).getDetail();

```

`一些基本物料不会变，而其组合经常变化的时候`，就可以选择这样的设计模式来构建代码。将复杂的构建逻辑封装在了 Builder 内部，客户端无需关心，链式调用代码，意图非常清晰。
